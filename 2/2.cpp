...
bool gameValid(const Game& game) {
    std::map<std::string, int> valid_critera = {
        {"red", 12}, {"green", 13}, {"blue", 14},
    };
    for (std::map<std::string, int> g : game.games) {
        // iterate through all colors in the game and check with the valid critera
		std::map<std::string, int>::iterator it;
		for (it = g.begin(); it != g.end(); ++it) {
			const std::string& key = it->first;
			if (valid_critera[key]) {
				if (it->second > valid_critera[key]) {
                    return false;
                }
			} else {
				return false;
...
int main() {
    ...
    while (std::getline(inputFile, line)) {
        ...
        if (gameValid(game)) {
            std::cout << "game " << game.gameNumber << " is valid" << std::endl;
            sum += game.gameNumber;
        }
    }
