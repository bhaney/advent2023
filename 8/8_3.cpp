...
int main() {
    ...
    while (!allZNodes(nodes)) {
        count += 1;
        for(int i = 0; i < nodes.size(); i++) {
            // check if every node has entered a cycle yet
            // once all nodes are in a cycle, break out of the loop, and manually calculate answer
        }
