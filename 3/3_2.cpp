...
int findNumber(int i, int length, std::string& line) {
    ...
    std::string replacement(num_len, '.'); // still replace here in order to not double count
...
int scanLines(std::string& prev_line, std::string& curr_line, std::string& next_line) {
    // this time we dont modify the lines, one number can be shared by two symbols
    int sum = 0;
...
int main() {
    ...
    while (std::getline(inputFile, line)) {
        ...
        // if we are line_num = 1, it means we are on the first line
